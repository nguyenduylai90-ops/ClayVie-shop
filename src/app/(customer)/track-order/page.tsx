  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-20">
      <div className="text-center mb-12">
        ...
        <h1 className="text-4xl font-black text-gray-900 mb-4 tracking-tighter">Tra cứu đơn hàng</h1>
      ...
      <div className="space-y-10">
        {searched && orders.length === 0 && !loading && (
          <div className="text-center py-16 bg-gray-50 rounded-[40px] border-4 border-dashed border-gray-100">
            ...
            <p className="text-gray-400 font-bold text-xl">Chưa có đơn hàng cho số điện thoại này</p>
          </div>
        )}
            ...
            <div key={order.id} className="bg-white p-8 rounded-[40px] border border-gray-100 shadow-sm hover:shadow-2xl transition-all duration-500">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8 bg-gray-50 p-6 rounded-3xl border border-gray-100">
                <div className="text-center md:text-left">
                  <p className="text-[10px] text-gray-400 font-bold uppercase mb-1">Tổng tiền</p>
                ...
                <div className="text-center md:text-right">
                  ...
                  <p className={`font-black text-2xl ${remaining <= 0 ? 'text-green-500' : 'text-pink-600'}`}>
                    {remaining <= 0 ? '✅ Đã hoàn tất' : `${remaining.toLocaleString('vi-VN')}đ`}
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 gap-6 border-t border-gray-50 pt-8">
                <div>
                  <p className="text-[10px] text-gray-400 font-bold mb-1 uppercase tracking-wider">Người nhận & Địa chỉ giao hoa</p>
                  <p className="font-bold text-gray-800 text-lg">{order.customer_name}</p>
                  <p className="text-gray-500 mt-1">{order.customer_address}</p>
                </div>
                
                {order.customer_notes && (
                  <div className="bg-pink-50/50 p-4 rounded-2xl border border-pink-100">
                    <p className="text-[10px] text-pink-400 font-bold uppercase mb-1">Yêu cầu thêm của bạn</p>
                    <p className="text-gray-700 italic">"{order.customer_notes}"</p>
                  </div>
                )}
                
                <p className="text-[10px] text-gray-300 font-bold uppercase text-center mt-4">Đặt ngày: {new Date(order.created_at).toLocaleDateString('vi-VN')}</p>
              </div>
            </div>
          );
        })}
