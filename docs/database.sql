...
-- 8. Trigger tự động CỘNG LẠI hàng tồn kho khi XÓA HẲN đơn hàng (Chạy BEFORE DELETE trên orders)
CREATE OR REPLACE FUNCTION public.restore_stock_before_order_delete()
RETURNS TRIGGER AS $$
DECLARE
  item RECORD;
BEGIN
  -- Chỉ cộng lại kho nếu đơn hàng bị xóa CHƯA ở trạng thái 'cancelled' 
  -- (Nếu đã hủy 'cancelled' thì kho đã được cộng trả trước đó)
  IF OLD.status != 'cancelled' THEN
    FOR item IN SELECT product_id, quantity FROM public.order_items WHERE order_id = OLD.id LOOP
      UPDATE public.products
      SET stock_quantity = stock_quantity + item.quantity
      WHERE id = item.product_id;
    END LOOP;
...
DROP TRIGGER IF EXISTS trigger_restore_stock_before_order_delete ON public.orders;
CREATE TRIGGER trigger_restore_stock_before_order_delete
BEFORE DELETE ON public.orders
FOR EACH ROW
EXECUTE FUNCTION public.restore_stock_before_order_delete();
